...
純「添加」用的 skill：把使用者**指定的內容**寫進 `~/.claude/mame/lessons.jsonl`（之後可用 `/mame:learn` 撈回、忙超過門檻秒數時在 statusLine 輪播）。和 `/mame:commit` 不同，這裡**不看 git、不做 commit、不自行判斷值不值得記** —— 使用者叫你記，你就記。
